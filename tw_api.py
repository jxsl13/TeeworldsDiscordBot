...
def get_server_info(address):
	...
	try:
		...
		if DEBUG:
			print(*e.args)
		server_info['error'] = str(*e.args)
	except OSError as e: # Timeout
		if DEBUG:
			print("> Server", address, "did not answer:", e)
		...
		if DEBUG:
			print("> Server", address, "did something wrong here")
		server_info['error'] = str(e)
		# import traceback
		# traceback.print_exc()
	finally:
...
class Master_Server_Info(threading.Thread):
	...
	def __str__(self):
		return f"Master Server: {self.address}"
...
def get_list(address):
	...
	try:
		...
		if not answer and DEBUG:
			print("> Master", address, "did not answer:", e)
	except Exception as e:
		if DEBUG:
			print(e)
		# import traceback
		...
		if servers_info[0].finished == True:
			if servers_info[0].info:
				...
				if   server_info["num_players"] > server_info["max_players"] \
					...
					print("> Server", server_info["address"], "has", server_info["bad"])
				# check actual purity
